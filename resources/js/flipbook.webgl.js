...

{
                ...
                this.options.main.loadPage(p1, size, function (page) {
                    ...
                    if (page.canvas) {
                        ...
                        if (self.options.pages[2 * self.index].side == "left") {
                            ...
                            self.options.pages[2 * self.index].side == "right"
                        ) {
                            t1.repeat.x = 0.5;
                            t1.offset.x = 0.5;
                        }
            ...
            if (this.sizeBack == size) {
                ...
                this.options.main.loadPage(p2, size, function (page) {
                    ...
                    if (page.canvas) {
                        ...
                        if (
                            self.options.pages[2 * self.index + 1].side ==
                            "left"
                        ) {
                            t2.repeat.x = 0.5;
                        } else if (
    ...
    FLIPBOOK.PageWebGL.prototype.createMaterial = function (map, side) {
        ...
        if (this.options.lights) {
            var sTexture =
                side == "back" ? this.book.specularB : this.book.specularF;
            var o = this.options;
            ...
            if (this.index < this.book.pages.length - 1) {
                ...
                if (this.index > 0)
                    this.book.pages[this.index].position.z =
                        this.book.pages[this.index - 1].position.z +
                        th / 2 +
                        th / 2 -
                        1;
            }
    ...
    FLIPBOOK.PageWebGL.prototype.bendIn = function (
        angle,
        newForce,
        newOffset
    ) {
    ...
    FLIPBOOK.PageWebGL.prototype.bendOut = function () {
        ...
        var self = this.thiss;
        var time =
            self.duration * Math.pow(Math.abs(self.bendF.force), 0.5) * 1000;
            ...
            if (self.index < self.book.pages.length - 1) {
                ...
                if (self.book.pages.length > 1)
                    self.book.pages[self.index].position.z =
                        self.book.pages[self.index - 1].position.z +
                        th / 2 +
                        th / 2 -
                        1;
            }
...
*/
    ...
    FLIPBOOK.BookWebGL.prototype.initHtmlContent = function () {
        ...
        this.$pageR = jQuery(document.createElement("div"))
            ...
            .css({
                width:
                    (1000 * this.options.pageWidth) / this.options.pageHeight +
                    "px",
                height: "1000px",
                position: "absolute",
                top: "-500px",
    ...
    FLIPBOOK.BookWebGL.prototype.isZoomed = function () {
        return (
            this.options.zoom > this.options.zoomMin && this.options.zoom > 1
        );
    };
    ...
    FLIPBOOK.BookWebGL.prototype.onSwipe = function (
        ...
        if (phase == "move" && fingerCount <= 1) {
            if (this.draggingBook) {
                this.centerContainer.position.x =
                    this.draggingBookStartX - distance;
                this.updateHtmlLayerPosition();
                return;
            }
    ...
    FLIPBOOK.BookWebGL.prototype.updateCameraPosition = function () {
        ...
        var cameraX = 0;
        var cameraY =
            (this.options.cameraDistance * Math.sin(angle)) / this.zoom;
        var cameraZ =
            (this.options.cameraDistance * Math.cos(angle)) / this.zoom;
    ...
    FLIPBOOK.BookWebGL.prototype.createPages = function () {
        ...
        for (i = 0; i < p.length / 2; i++) {
            ...
            if (i > 0) {
                page.position.z =
                    self.pages[i - 1].position.z - th / 2 - th / 2 - 1;
        ...
        if (this.rightIndex > index) {
            ...
            if (this.rightIndex - 2 > index) {
                ...
                setTimeout(function () {
                    if (typeof instant != "undefined" && instant) {
                        for (var i = 0; i < self.pages.length; i++) {
                            self.pages[i].duration =
                                self.options.pageFlipDuration;
                        }
                    }
    ...
    FLIPBOOK.BookWebGL.prototype.zoomTo = function (amount, time, x, y) {
        ...
        if (typeof x != "undefined" && typeof y != "undefined") {
            ...
            var focus = {
                ...
                y:
                    (-y + this.wrapper.clientHeight / 2) / scaleFactor -
                    center.y,
            };
            var focusNew = {
                x:
                    (x - this.wrapper.clientWidth / 2) / scaleFactorNew -
                    center.x,
                y:
                    (-y + this.wrapper.clientHeight / 2) / scaleFactorNew -
                    center.y,
            };
    ...
    FLIPBOOK.BookWebGL.prototype.tiltTo = function (amount) {
        ...
        newTilt =
            newTilt > this.options.tiltMax ? this.options.tiltMax : newTilt;
        newTilt =
            newTilt < this.options.tiltMin ? this.options.tiltMin : newTilt;
    ...
    FLIPBOOK.BookWebGL.prototype._touchmove = function (e) {
        ...
        if (e.touches.length > 1) {
            ...
            var newZoom =
                this.zoom *
                Math.pow(this.touchesDist / this.touchesDistStart, 0.2);
            if (
                newZoom >= this.options.zoomMin &&
        ...
        if (!this.mouseDown) {
            ...
            if (this.options.rotateCameraOnMouseMove) {
                ...
                this.pan =
                    ...
                    ((this.options.panMax2 - this.options.panMin2) *
                        this.pointX) /
...

{
            ...
            if (this.force != 0)
                for (
                    var c = this.mod.getVertices(),
                        ...
                        p =
                            a.Constants.doublePI *
                            (d / (k * a.Constants.doublePI)),
                        l,
                        j,
                        q,
                        r,
                    ...
                    (d = c[b]),
                        ...
                        (this.constraint == a.ModConstant.LEFT && r <= e) ||
                            (this.constraint == a.ModConstant.RIGHT &&
                                r >= e) ||
                            ((r = s - p * e + p * r),
                            (l = u(r) * (k + q)),
                            (r = v(r) * (k + q)),
    ...
    (function (a) {
        ...
        a.Twist.prototype.twistPoint = function (c, b) {
            var d = new a.Matrix4().translationMatrix(
                    c.getX(),
                    c.getY(),
                    c.getZ()
                ),
    ...
    (function (a) {
        ...
        a.Break.prototype.apply = function () {
            ...
            h = c.negate();
            for (
                i = new a.Matrix4().rotationMatrix(g.x, g.y, g.z, f);
                --d >= 0;
                ...
                (f = b[d]),
                    ...
                    g.z >= 0 &&
                        e.isIn(f.ratioY) &&
                        new a.Matrix4().multiplyVector(i, g),
                    (g = g.add(h)),
                    f.setX(g.x),
                    f.setY(g.y),
                    f.setZ(g.z);
        ...
        function renderObject(object, camera, cameraCSSMatrix) {
            if (object instanceof FLIPBOOK.CSS3DObject) {
                ...
                if (cachedStyle === undefined || cachedStyle !== style) {
                    ...
                    if (isIE) {
                        objectData.distanceToCameraSquared =
